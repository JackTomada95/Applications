...
class AverageVehicleModelBrand():

    def __init__(self):
        
        global data
        data = []
        colHeadings = ["Vehicle ID", "Model", "Brand", "Average"]
        d0 = ["","","", ""]
        data.append(d0)
    ...
    def tablePopulate(self):
        ...
        try:
            ...
            selectFactTableRows = "SELECT ve.idv, ve.model, ve.brand, ROUND(AVG(rp.revenue), 2) AS average FROM \"Garages\".\"Vehicle\" ve JOIN \"Garages\".\"RepairFacts\" rp ON ve.idv = rp.idv GROUP BY ROLLUP (ve.brand, ve.model, ve.idv);"
            ...
            sg.Popup("ROLLUP VEHICLE BY MODEL-BRAND (AVERAGE)", str(error))
