...
class AverageCustomerLocation():

    def __init__(self):
        
        global data
        data = []
        colHeadings = ["Customer ID", "City", "Region", "Average"]
        d0 = ["","","", ""]
        data.append(d0)
    ...
    def tablePopulate(self):
        ...
        try:
            ...
            selectFactTableRows = "SELECT cu.idc, cu.city, cu.region, ROUND(AVG(rp.revenue), 2) AS average FROM \"Garages\".\"Customer\" cu JOIN \"Garages\".\"RepairFacts\" rp ON cu.idc = rp.idc GROUP BY ROLLUP (cu.region,cu.city,cu.idc);"
            ...
            sg.Popup("ROLLUP CUSTOMER BY LOCATION (AVERAGE)", str(error))
