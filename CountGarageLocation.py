...
class CountGarageLocation():

    def __init__(self):
        
        global data
        data = []
        colHeadings = ["Garage ID", "City", "Region", "Count"]
        d0 = ["","","", ""]
        data.append(d0)
    ...
    def tablePopulate(self):
        ...
        try:
            ...
            selectFactTableRows = "SELECT ga.idg, ga.city, ga.region, COUNT(rp.revenue) FROM \"Garages\".\"Garage\" ga JOIN \"Garages\".\"RepairFacts\" rp ON ga.idg = rp.idg GROUP BY ROLLUP (ga.region,ga.city,ga.idg);"
            ...
            sg.Popup("ROLLUP GARAGE BY LOCATION (COUNT)", str(error))
