...
class AverageGarageLocation():

    def __init__(self):
        
        global data
        data = []
        colHeadings = ["Garage ID", "City", "Region", "Average"]
        d0 = ["","","", ""]
        data.append(d0)
    ...
    def tablePopulate(self):
        ...
        try:
            ...
            selectFactTableRows = "SELECT ga.idg, ga.city, ga.region, ROUND(AVG(rp.revenue), 2) AS average FROM \"Garages\".\"Garage\" ga JOIN \"Garages\".\"RepairFacts\" rp ON ga.idg = rp.idg GROUP BY ROLLUP (ga.region,ga.city,ga.idg);"
            ...
            sg.Popup("ROLLUP GARAGE BY LOCATION (AVERAGE)", str(error))
