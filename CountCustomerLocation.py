...
class CountCustomerLocation():

    def __init__(self):
        
        global data
        data = []
        colHeadings = ["Customer ID", "City", "Region", "Count"]
        d0 = ["","","", ""]
        data.append(d0)
    ...
    def tablePopulate(self):
        ...
        try:
            ...
            selectFactTableRows = "SELECT cu.idc, cu.city, cu.region, COUNT(rp.revenue) FROM \"Garages\".\"Customer\" cu JOIN \"Garages\".\"RepairFacts\" rp ON cu.idc = rp.idc GROUP BY ROLLUP (cu.region,cu.city,cu.idc);"
            ...
            sg.Popup("ROLLUP CUSTOMER BY LOCATION (COUNT)", str(error))
